...
model = genai.GenerativeModel(
    ...
    system_instruction="""你是 SQL Server T-SQL 專家。
你的任務是根據使用者提供的 Schema 及中文問題，產生正確的 T-SQL 查詢語句。

規則：
1. 只輸出 SQL 語句本身，不要加任何說明文字或 Markdown 標記（不要有 ```sql 等符號）。
...
def nl_to_sql(question: str, schema_context: str) -> str:
    """Convert a natural language question to T-SQL using Gemini."""
    prompt = f"""資料庫：gemio10
